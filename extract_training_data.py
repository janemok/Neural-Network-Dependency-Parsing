...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        for i in range(-1,-4,-1):
            if track > len(state.stack):
                arr[indx] = self.word_vocab["<NULL>"]
            else:
                if pos[state.stack[i]] == "CD":
                    arr[indx] = self.word_vocab['<CD>']
                elif pos[state.stack[i]] == "NNP":
                    arr[indx] = self.word_vocab['<NNP>']
                elif words[state.stack[i]] == None:
                    arr[indx] = self.word_vocab['<ROOT>']
                elif words[state.stack[i]].lower() in self.word_vocab:
                    arr[indx] = self.word_vocab[words[state.stack[i]].lower()]
                else: 
                    arr[indx] = self.word_vocab['<UNK>']                
            track += 1
            indx += 1

        #buffer
        for i in range(-1,-4,-1):
            if track-3 > len(state.buffer):
                arr[indx] = self.word_vocab["<NULL>"]
            else:
                if pos[state.buffer[i]] == "CD":
                    ...
                    arr[indx] = self.word_vocab['<NNP>']
                elif words[state.buffer[i]] == None:
                    arr[indx] = self.word_vocab['<ROOT>']
                elif words[state.buffer[i]].lower() in self.word_vocab:
                    arr[indx] = self.word_vocab[words[state.buffer[i]].lower()]
                else: 
                    arr[indx] = self.word_vocab['<UNK>']     
            track += 1
            indx += 1
